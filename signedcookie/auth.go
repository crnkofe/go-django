...
// into a string.  Currently two serializers are supported, JSON.
type Serializer int
...
func unsign(secret string, cookie []byte, salt string) ([]byte, error) {
...
func timestampUnsign(maxAge time.Duration, secret string, cookie []byte, salt string) ([]byte, error) {
	val, err := unsign(secret, cookie, salt)
	if err != nil {
...
func signingLoads(s Serializer, maxAge time.Duration, secret, cookie string, salt string) (map[string]interface{}, error) {
	...
	payload, err := timestampUnsign(maxAge, secret, c, salt)
	if err != nil {
...
func Decode(s Serializer, maxAge time.Duration, secret, cookie string, salt string) (map[string]interface{}, error) {
	return signingLoads(s, maxAge, secret, cookie, salt)
}
